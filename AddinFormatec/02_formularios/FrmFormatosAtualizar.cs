...
namespace AddinFormatec {
  public partial class FrmFormatosAtualizar : LmChildForm {
    ...
    private void Atualizar() {
      try {
        Invoke(new MethodInvoker(() => {
          ...
          swApp.OpenDoc6(FormatoFolha.model.FormatoA4R, (int)swDocumentTypes_e.swDocDRAWING,
              (int)swOpenDocOptions_e.swOpenDocOptions_Silent, "", ref status, ref warnings);

          swApp.ActivateDoc2(FormatoFolha.model.FormatoA4R, false, 0);
          swModelTemplate = (ModelDoc2)swApp.ActiveDoc;
          ...
          for (int i = dadosDesenho.Position; i <= dadosDesenho.Count; i++) {
            ...
            if (file.Atualizar) {
              try {
                swApp.OpenDoc6(file.PathName, (int)swDocumentTypes_e.swDocDRAWING,
                                       (int)swOpenDocOptions_e.swOpenDocOptions_Silent, "", ref status, ref warnings);
              } catch (Exception ex) {
              ...
              swApp.ActivateDoc2(file.PathName, false, 0);
              swModel = (ModelDoc2)swApp.ActiveDoc;

              ChangeFileProps(swModel, swModelTemplate);
              UpdateFormato(swApp, swModel);

              swModel.Save();
              swApp.CloseDoc(file.PathName);
            }
          }

          swApp.CloseDoc(FormatoFolha.model.FormatoA4R);

          if (imprimindo) {
            Toast.Success("Formatos de folha atualizados com sucesso!");
    ...
    public static void UpdateFormato(SldWorks swApp, ModelDoc2 swModel) {
      try {
        DrawingDoc swDraw;
    ...
    private static void InserirListasMateriais(DrawingDoc swDraw, ModelDoc2 swModel) {
      try {
        ...
        if (tipo == "peça") {
          WMTable = swView.InsertWeldmentTable(true, 0, 0, AnchorType, activeConfiguration, Config.model.ListaPeca);
        } else {
          swBOMAnnotation = swView.InsertBomTable3(true, 0, 0, AnchorType, BomType, activeConfiguration, Config.model.ListaMontagem, false);
        }
      } catch (Exception ex) {
